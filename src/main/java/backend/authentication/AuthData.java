...
public class AuthData {
    ...
    public int registerUser(User user, String password) {
        ...
        try(Connection con = DriverManager.getConnection(DatabaseConnection.connectionUrl);Statement statement = con.createStatement();PreparedStatement pstmt = con.prepareStatement("INSERT INTO [User] (fullName, email, phonenumber, password) VALUES (?,?,?,?)", Statement.RETURN_GENERATED_KEYS)) {
            pstmt.setString(1, user.getFullName());
            pstmt.setString(2, user.getEmail());
            pstmt.setString(3, user.getPhoneNumber());
            pstmt.setString(4, Password.hashPassword(password));
